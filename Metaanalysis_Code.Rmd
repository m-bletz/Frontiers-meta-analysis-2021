...
forest(Lineage_basemod, atransf=exp, at=log(c(.05, .25, 1, 4,10,40)), xlim=c(-16,10), cex=.75, header="Study ID", mlab="")
...
forest(Lineage_basemod_noOL_LS, xlim=c(-16, 10), at=log(c(0.05, 0.25, 1, 4,10)), atransf=exp,
      cex=0.6, ylim=c(-1, 28),
...
text(-16, c(23,18,10), pos=4, c("Metamorph",
                               "Larvae*",
                               "Adult"))
...
addpoly(Lineage_basemod_noOL_LSl, row= 11, cex=0.75, atransf=exp, mlab=mlabfun("RE Model for Subgroup", Lineage_basemod_noOL_LSl))
addpoly(Lineage_basemod_noOL_LSm, row= 19, cex=0.75, atransf=exp, mlab=mlabfun("RE Model for Subgroup", Lineage_basemod_noOL_LSm))
...
**Effects for Sugroups**

Risk Ratio for Adults

```{r}
exp(coef(Lineage_basemod_noOL_LS)[[1]])

predict(Lineage_basemod_noOL_LS, newmods=c(0,0), transf=exp, digits=2)
```

Risk Ratio of Larvae*

```{r}
exp(coef(Lineage_basemod_noOL_LS)[[1]] + coef(Lineage_basemod_noOL_LS)[[2]])

predict(Lineage_basemod_noOL_LS, newmods=c(1,0), transf=exp, digits=2)
```

How much greater is larval risk vs adult?

```{r}
exp(coef(Lineage_basemod_noOL_LS)[[2]])
...
exp(coef(Lineage_basemod_noOL_LS)[[1]] + coef(Lineage_basemod_noOL_LS)[[3]])
```

How much greater is metamorph risk vs adult?

```{r}
exp(coef(Lineage_basemod_noOL_LS)[[3]])
```

How much greater is metamorph risk vs larvae?

```{r}
(exp(coef(Lineage_basemod_noOL_LS)[[1]] + coef(Lineage_basemod_noOL_LS)[[3]])) / (exp(coef(Lineage_basemod_noOL_LS)[[1]] + coef(Lineage_basemod_noOL_LS)[[2]]))
...
# Hat value outliers < 3*(1/k) == 3*(1/23) == 0.13 
...
Outliers: **1** Alytes muletensisTF5a1-UKTvB
...
<span style="color: red;"> With the updated data set host eco and life stage are "redundant predictors" in the full model </span>
...
**Conclusion** Full model is "slightly better" than intercept only model based on AIC only
*random study term included in both*
...
**Conclusion:** RR of the non-GPL.GPL group is 0.49?
...
**Conclusion:** RR of the GPL-GPL group is 1.01?
...
<span style="color: red;"> Perhaps the above is over complicating things and actually what we want to know is if we subset to just GPL-GPL, which is easier in terms of framing and interpretation (I think thats what I recall from last meeting? :) ), whether we see the same results... And the answer to that is yes.  RR for full data is 1.04 and not significant vs 1.01 for GPL-GPL only and model is not significant (coded below) </span>
